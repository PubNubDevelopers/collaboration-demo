...
document.getElementById('colorSwatch').addEventListener(
  'click',
  function () {
...
document
  .getElementById('clearAllCanvasButton')
  .addEventListener('click', function () {
    ctx.clearRect(0, 0, canvas.width, canvas.height)
    publish({ clear: true })
...
function drawOnCanvas (color, startCoordinates, endCoordinates) {
  ...
  if (typeof CLEAR_LINE_OFFSET_DURATION !== 'undefined') {
    setTimeout(function () {
      clearLine(startCoordinates, endCoordinates)
    }, CLEAR_LINE_OFFSET_DURATION)
  }
}
...
function drawFromStream (message) {
  ...
  for (var i = 0; i < message.data.length; i++) {
    drawOnCanvas(
      message.data[i].color,
      message.data[i].oldCoordinates,
      message.data[i].newCoordinates
    )
  }
}
...
function draw (e) {
  ...
  if ((isTouchSupported && e.touches.length == 1) || e.buttons == 1) {
    ...
    var x = isTouchSupported
      ? e.targetTouches[0].pageX + 10
      : e.offsetX /* - 10 || e.layerX - canvas.offsetLeft*/
    var y = isTouchSupported
...
function Sprite (state) {
  ...
  this.update = function () {
    //Updates the user mouse position by controlling speed and animation.
...
async function updateUser (uuid, state) {
  if (!users[uuid]) {
    ...
    try {
      if (
        uuid == pubnub.getUUID() &&
        typeof USE_OBJECTS_METADATA !== 'undefined' &&
        USE_OBJECTS_METADATA == true
      ) {
